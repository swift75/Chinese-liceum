...
class User(UserMixin, database.Model):
    ...
    points = database.Column(database.Integer, default=0, nullable=False)
...
class ChineseWord(database.Model):
    ...
    example_sentence = database.Column(database.Text)
    example_translation = database.Column(database.Text)
